...
class StatusBar(tk.Frame):
    ...
    NOTE_STATUS_COLORS = ["#D7191C", "#FDAE61", "#FFFFBF", "#ABD9E9",
                          "#2C7BB6"]
    ...
    def __init__(self, master):
        ...
        self.centre_status = tk.Label(
            self, relief=tk.SUNKEN, anchor=tk.W, width=35)
        self.centre_status.pack(side=tk.LEFT, fill=tk.X, padx=5)

        self.note_status = tk.Label(
            self, relief=tk.SUNKEN, anchor=tk.W, width=25)
...
class NotesList(tk.Frame):
    ...
    def __init__(self, master, font_family, font_size, config):
        ...
        self.text.tag_config("found", font=italic_font,
                             foreground="dark gray", background="lightyellow")
    ...
    def append(self, note, config):
        ...
        if self.layout == "vertical" and self.print_columns == 1:
            ...
            self.text.insert(tk.END, u'{0:<{w}}'.format(
                title[:cellwidth - 1], w=cellwidth), ("title,"))

            if tags > 0:
                if config.tagfound:
                    self.text.insert(tk.END, u'{0:<{w}}'.format(','.join(
                        tags)[:cellwidth - 1], w=cellwidth), ("found",))
                else:
                    self.text.insert(tk.END, u'{0:<{w}}'.format(','.join(
                        tags)[:cellwidth - 1], w=cellwidth), ("tags",))

            self.text.insert(
                tk.END, ' ' + utils.human_date(modifydate), ("modifydate",))
    ...
    def select(self, idx, silent=True):
        ...
        # remove tag selected from row 1 (first) and column 0 to the end of the
        # buffer
...
tkinter_umlauts = ['odiaeresis', 'adiaeresis', 'udiaeresis', 'Odiaeresis',
...
class View(utils.SubjectMixin):
    ...
    def _bind_events(self):
        ...
        self.root.bind_all(
            "<Control-question>", lambda e: self.cmd_help_bindings())
        ...
        self.root.after(
            self.config.housekeeping_interval_ms, self.handler_housekeeper)
    ...
    def _create_ui(self):
        ...
        cs_checkbutton = tk.Checkbutton(
            search_frame, variable=self.cs_checkbutton_var)
    ...
    def handler_search_enter(self, evt):
        ...
        if self.notes_list.selected_idx >= 0:
            ...
            self.notify_observers('create:note', utils.KeyValueObject(
                title=self.get_search_entry_text()))
            # the note will be created synchronously, so we can focus the text
            # area already
            self.text_note.focus()
    ...
    def set_notes(self, notes):
        ...
        for o in notes:
            ...
            self.notes_list.append(
                o.note, utils.KeyValueObject(tagfound=o.tagfound))
